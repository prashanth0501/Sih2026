...
export function FlipCard({
  ...
  return (
    ...
    >
      <div
        className={cn(
          'group relative h-full w-full rounded-xl transition-transform duration-500 [transform-style:preserve-3d]',
          flipped && '[transform:rotateY(180deg)]'
        )}
        style={{ transitionTimingFunction: 'cubic-bezier(.2,.8,.2,1)' }}
      >
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-xl border border-line bg-paper-2 p-6 text-center [backface-visibility:hidden]">
          {photoUrl ? (
        ...
        <div className="absolute inset-0 flex flex-col items-start justify-start gap-2 rounded-xl border border-line bg-ink p-6 text-left text-paper [backface-visibility:hidden] [transform:rotateY(180deg)]">
